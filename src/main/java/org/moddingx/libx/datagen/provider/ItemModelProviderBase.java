...
public abstract class ItemModelProviderBase extends ItemModelProvider {
    ...
    protected void defaultItem(ResourceLocation id, Item item) {
        if (item instanceof SpawnEggItem) {
            ...
            this.withExistingParent(id.getPath(), DRIPPING_BUCKET).texture("base", this.modLoc("item/" + id.getPath())).customLoader(DynamicFluidContainerModelBuilder::begin).fluid(bucketItem.getFluid());
        } else {
    ...
    private static boolean isItemStackRenderer(Item item) {
        try {
            IClientItemExtensions properties = IClientItemExtensions.of(item);
            if (properties != IClientItemExtensions.DEFAULT) {
                properties.getCustomRenderer();
            } else {
                ...
                properties = ref.get();
                if (properties != null) properties.getCustomRenderer();
            }
